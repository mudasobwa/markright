          ...
          Enum.each(0..@max_indent, fn indent ->
            Enum.each(Markright.Syntax.block(@syntax), fn {tag, {delimiter, _opts}} ->
              ...
              defp astify(
                   ...
                   when not (rest == "") do
                # TODO: extract this with into Utils fun
            ...
            defp astify(rest, %Plume{} = plume) when is_binary(rest) do
              with %Plume{} = cont <-
                     apply(plume.bag[:parser], :to_ast, [@unix_newline <> rest, plume]) do
                ...
                %Plume{
                  ...
                  | ast: [
                      Markright.Utils.continuation(
                        :ast,
                        %Plume{cont | ast: trim_leading(mine)},
                        {:p, %{}}
                      ),
                      rest
                    ],
          ...
          Enum.each(~w|] ) }|, fn delimiter ->
            ...
            defp astify(<<@delimiter::binary, rest::binary>>, %Plume{} = plume) do
              with {type, plume} <- Plume.debag!(plume, :type) do
                # FIXME REMOVE type
                %Plume{plume | tail: "", ast: {nil, %{type => plume.tail}, rest}}
              end
            end
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      with mod when is_atom(mod) and not is_nil(mod) <- opts[:modules][:block],
        ...
        _ ->
          raise Markright.Errors.UnexpectedModule,
            value: opts[:modules][:block],
            expected: :behaviour
      end
      ...
      defp astify!(:split, tag, {plain, rest, %Plume{} = plume}) do
        with %Plume{ast: pre_ast, tail: ""} = plume <- astify(plain, plume),
          ...
          Plume.continue(
            Plume.untail!(plume),
            Markright.Utils.join!([pre_ast, {tag, attrs, ast}, post_ast]),
            tail
          )
        end
      end
      ...
      defp astify!(:custom, tag, {plain, rest, %Plume{} = plume}) do
        with mod <- Markright.Utils.to_parser_module(tag),
          ...
          Plume.continue(
            Plume.untail!(plume),
            Markright.Utils.join!([pre_ast, ast, post_ast]),
            tail
          )
        end
      end
      ...
      defp astify!(:join, _tag, {plain, rest, %Plume{} = plume}) do
        with %Plume{ast: pre_ast, tail: more} <- astify(plain, plume),
             plume <- plume |> Plume.untail!(),
             %Plume{ast: post_ast, tail: tail} <-
               apply(@block_module, :to_ast, [more <> rest, plume]) do
      ...
      defp astify!(type, tag, {plain, rest, %Plume{} = plume}) do
        with mod <-
               Markright.Utils.to_parser_module(tag,
                 fallback: Markright.Utils.to_parser_module(type)
               ),
